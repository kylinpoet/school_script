...
def read_user_data(file_path='teacher_info.csv') -> list:
    '''
...
class imageCode2str:
    ...
    def __init__(self, img_data) -> None:
        ...
        self.tessconfig = r'--tessdata-dir "D:\\soft\\jTessBoxEditor\\tesseract-ocr\\tessdata"  \
    ...
    def image2str(self):
        ocr_result = pytesseract.image_to_string(
            self.image_binaryzation(), lang='zjer', config=self.tessconfig)
...
def user_login(userid='', userpass='', try_time=__try_time):
            ...
            if resp_code == '000000':
                ...
                for k in range(3):
                    ...
                    if resp_code == '000000':
                        ...
                        print(Fore.CYAN + f'验证码第二轮尝试!重试第{k+1}次!')
                        if k==2:
                            raise Exception('验证码第二次错误')
                        continue
...
def visit_exp(req_s:requests.sessions.Session):
    for i in range(1, __try_time + 1):
        ...
        try:
            ...
            for user in users:
                resp = req_s.get(f'{url}{user}', headers=headers, timeout=25)
                time.sleep(3)
            print_time_line()
...
def act_exp():
    ...
    for user in user_all:
        req_session = user_login(user['account'], user['password'])
...
if __name__ == '__main__':
    ...
    schedule.every(61).to(71).minutes.do(act_login)
