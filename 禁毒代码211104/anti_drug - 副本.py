...
def quiz(req_session, reqtoken):
    ...
    # exam_data['list'][8]['questionContent'] = random.choice(['A','B'])
    # exam_data['list'][9]['questionContent'] = random.choice(['A', 'B'])
    # exam_data['list'][10]['questionContent'] = random.choice(['A', 'B'])
    for i in range(1, __try_time + 1):
        try:
            response = req_session.post(url=_url,
                                        headers=headers1,
            ...
            if dict_resp.get('success'):
                try:
                    ...
                    response = req_session.get(_url, headers=headers)
                    dict_resp = response.json()
    ...
    if courselist:
        for course in courselist:
            ...
            for i in range(__try_time):
                try:
                    testPaperList = req_session.get(_TestPaperList_url).json()['data'].get('testPaperList')
                    break
                except Exception as e:
                    print(Fore.RED + ' 播放视频失败,错误为：' + str(e))
                    if i <= __try_time-1:
